...
import pandas as pd
import argparse
import platform, sklearn, imblearn, scipy
...
from ml_utils.io_utils import load_data, ensure_sex_encoded, encode_target, select_feature_blocks
from ml_utils.models import iter_model_specs
from ml_utils.importance_utils import (
    compute_basic_metrics, save_feature_importances, shap_top_features,
    save_calibration_table, expected_calibration_error,
    conditional_group_permutation_importance, safe_auc_scorer,
    _mean_std_ci, _fmt,
)
...
try:
    ...
    _shap_ver = getattr(_shap, "__version__", "not installed")
except Exception:
    _shap_ver = "not installed"
...
def data_loading_and_preprocessing(csv_path: Path, args) -> Dict[str, Any]:
    ...
    return {
        "df": df, "y": y, "groups": groups, "class_names": class_names,
        "pre": pre, "norm": norm, "micro_cols": micro_cols, "covs": covs
    }

def build_model_specs(seed: int, skip: List[str]) -> List[Tuple[str, Any, Dict[str, Any]]]:
    return [(n, e, g) for (n, e, g) in iter_model_specs(seed=seed)
...
def run_nested_cv_and_eval(
    ...
    outer_cv = StratifiedGroupKFold(n_splits=n_outer, shuffle=True, random_state=args.seed)
    inner_cv = StratifiedGroupKFold(n_splits=n_inner, shuffle=True, random_state=args.seed)
    ...
    for name, estimator, param_grid in model_specs:
        ...
        for tr_idx, va_idx in outer_splits:
            ...
            est, search_obj = tune_or_fit_inner(
                name, estimator, param_grid, X_tr, y_tr, groups_tr, inner_cv, pre, args,
            ...
            pr_fold_vals = []
            if (proba is not None) and ((proba.ndim == 2 and proba.shape[1] == n_classes_all) or n_classes_all == 2):
                for k in range(n_classes_all):
                    try:
                        p_k = (proba[:, 1] if (n_classes_all == 2 and proba.shape[1] == 2 and k == 1)
                               else (1.0 - proba[:, 1] if (n_classes_all == 2 and proba.shape[1] == 2 and k == 0)
                               else proba[:, k]))
                        ...
                        pr_fold_vals.append(np.nan); pr_auc_by_class[k].append(np.nan)
            ...
            aucs.append(float(auc)); accs.append(float(acc)); pr_aucs_macro.append(float(prc_macro))
            folds_used += 1
            ...
            if n_classes_all == 2:
                ...
                if proba is not None and proba.ndim == 2 and proba.shape[1] == n_classes_all:
                    oof_proba[va_idx, :] = proba
        ...
        try:
            ...
            oof_metrics = {"roc_auc": float("nan"), "pr_auc": float("nan"), "brier": float("nan"), "accuracy": float("nan")}
        ...
        per_model_details.append({
            ...
            "folds": folds_used,
            "fold_metrics": {
                "auc": aucs, "acc": accs, "pr_macro": pr_aucs_macro,
    ...
    best_name, best_trained, best_auc, best_acc, best_pr = max(candidates, key=lambda t: t[2])
    best_params = getattr(best_trained, "best_params_", {}) if hasattr(best_trained, "best_params_") else {}
    return per_model_details, best_name, best_auc, best_acc, best_pr, oof_store, best_trained, best_params
...
def posthoc_and_reporting(csv_path: Path, args, pack):
    ...
    """
    (df, y, groups, class_names, pre, norm, micro_cols, covs,
     per_model_details, best_name, best_auc, best_acc, best_pr, oof_store,
     best_trained, best_params) = pack
    ...
    try:
        ...
        if best_oof is not None:
            ...
            proba = best_oof["proba"]; pred = best_oof["pred"]
    ...
    try:
        if proba_full is not None and (proba_full.ndim == 1 or (proba_full.ndim == 2 and proba_full.shape[1] == 2)):
            save_calibration_table(y, proba_full, bins=10,
                                   out_csv=str(out_dir / f"{csv_path.stem}_{args.norm}_{best_name}_calibration.csv"))
    ...
    for tr, te in logo.split(df, y, groups):
        ...
        loo_hits += int(np.sum(yp == y[te])); loo_total += len(te)
    loo_acc = float(loo_hits / loo_total) if loo_total else float("nan")

    def _triplet(xs): return _fmt(*_mean_std_ci(xs))
    md = []
    md.append(f"# Evaluation Report — `{csv_path.stem}`\n")
    md.append(f"**Software:** Python {platform.python_version()}; scikit-learn {sklearn.__version__}, "
            f"imbalanced-learn {imblearn.__version__}, SciPy {scipy.__version__}, "
            f"NumPy {np.__version__}, pandas {pd.__version__}; SHAP {_shap_ver}.")
    md.append("**Validation:** nested CV — outer 5-fold (unbiased, group-aware), inner 3-fold (GridSearchCV on outer-train, group-aware). "
    ...
    if fold:
        ...
        for k, cname in enumerate(fold["class_names"]):
            md.append(f"- **PR AUC (CV), class {cname}:** {_triplet(fold['pr_by_class'][k])}")
        ...
        for metric_key, title in [
            ...
            for k, cname in enumerate(fold["class_names"]):
                md.append(f"- **{title}, class {cname}:** {_triplet(fold[metric_key][k])}")
        md.append("")
    ...
    (out_dir / "best_params.json").write_text(json.dumps({"model": best_name, "params": best_params}, ensure_ascii=False, indent=2), encoding="utf-8")
...
def run_for_file(csv_path: Path, args):
    ...
    (per_model_details, best_name, best_auc, best_acc, best_pr,
     oof_store, best_trained, best_params) = run_nested_cv_and_eval(
        d["df"], d["y"], d["groups"], d["class_names"], d["pre"], args, model_specs
    )

    pack = (d["df"], d["y"], d["groups"], d["class_names"], d["pre"], d["norm"],
            d["micro_cols"], d["covs"],
            per_model_details, best_name, best_auc, best_acc, best_pr, oof_store,
            best_trained, best_params)
...
def parse_args():
    ...
    p.add_argument("--norm", choices=["log10", "CLR", "DEICODE"], default="CLR",
    ...
    p.add_argument("--corr-threshold", type=float, default=0.98,
                   help="|corr(CLR)| threshold")
    p.add_argument("--prefer", choices=["prevalence", "variance"], default="prevalence")
    p.add_argument("--deicode-components", type=int, default=50,
                   help="DEICODE RPCA components")
